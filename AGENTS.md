...
2. **Never merge a PR without the captain's explicit word.**
   Never create, update, or merge a PR in any GitHub or other public repository the captain does not personally own unless the captain explicitly authorizes that exact repository.
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   The secondmate liveness sweep deterministically accounts for every registered secondmate: it fresh-spawns only from the recovery-grade `dead` or `missing` states, transactionally relaunches one attributed Herdr Claude process whose exact argv lost the permission posture its own launch recorded, preserves ambiguous, unreadable, or unreachable remote targets, and reports skipped or failed guarantees as `SECONDMATE_LIVENESS:` lines (`bin/fm-bootstrap.sh`; `bin/fm-backend.sh`'s `fm_backend_agent_state`; `docs/remote-secondmates.md`).
...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the away posture (`state/.afk-contract`, plus the `state/.afk` daemon flag where a daemon runs); and one cheap alive/dead read of each task's recorded backend endpoint.
   That liveness line is normally a fast presence check only, not a full state read; a live Claude-on-Herdr endpoint whose record carries its launch's permission mode gets one narrow posture read from that same process snapshot as well, so runtime restoration cannot hide manual mode. When you need a crew's actual current state (a run-step, not just "is the pane there"), read it with `bin/fm-crew-state.sh <id>` as before; the digest deliberately skips the broader, slower read for every task so it stays fast and bounded.
...
For an ordinary direct report whose endpoint is dead, whose Claude process reports permission drift, or whose metadata has no window, load `stuck-crewmate-recovery` and preserve the recorded worktree and unlanded work while reconciling ownership.
