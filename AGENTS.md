...
2. **Never merge a PR without the captain's explicit word.**
   Never create, update, or merge a PR in any GitHub or other public repository the captain does not personally own unless the captain explicitly authorizes that exact repository, including when that project's captain-approved `yolo` posture is on, because `yolo` never relaxes this ownership gate.
