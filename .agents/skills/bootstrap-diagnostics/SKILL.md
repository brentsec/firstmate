...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>|relaunch failed after <cause>: <reason>` - the session-start liveness sweep could not guarantee that the registered secondmate is running a real agent process.
  ...
  A `not replaced after the live Claude process lacks the permission posture its launch recorded` reason is the narrow live case: the mate is still running, and [`bin/fm-secondmate-restart.sh`](../../../bin/fm-secondmate-restart.sh) declined to replace it until it confirms its open work is written down, so the quoted outcome names what that owner did instead.
