...
- `SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>|relaunch failed after <cause>: <reason>` - the session-start liveness sweep could not guarantee that the registered secondmate is running a real agent process.
  Investigate the reason because that secondmate is not guaranteed live.
  A `so it was left running` reason is the narrow live case: the mate still holds its endpoint but its Claude process lost the permission posture its launch recorded, and the sweep refused to stop a worker it could not prove is between turns, so run the supervised replacement command that reason names.
  A `relaunch failed after` reason is never that case: the replacement was already attempted, so the old worker may be gone and the endpoint must be inspected rather than assumed live.
- `SECONDMATE_LIVENESS: secondmate <id>: relaunched after <cause> (<where>)` - the sweep replaced a live worker in its own endpoint and worktree, which supersedes any endpoint record an earlier digest printed for it.
