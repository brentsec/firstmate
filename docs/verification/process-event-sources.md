...
The live Bearings guard above exercises the real provider session contract without opening or focusing a desktop browser, ends only its exact temporary session, synchronously retires the listener in its isolated test home before removing the artifact, and never stops the shared Lavish server or closes unrelated review tabs.
