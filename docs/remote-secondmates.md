...
The startup liveness sweep does not repeat any of that for a remote Claude process that lost its recorded permission posture: such a process is still live, so the sweep hands the whole replacement to [`bin/fm-secondmate-restart.sh`](../bin/fm-secondmate-restart.sh), which persists the mate's open records first and then applies exactly the profile resolution and guards above.
