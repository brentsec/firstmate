...
The startup liveness sweep does not repeat any of that for a remote Claude process that lost its recorded permission posture: such a process is still live, so the sweep first asks the host for that worker's turn state through the `observe` verb, leaves a busy or unprovable worker running and reports the supervised route, and replaces a proven-idle one through the same `bin/fm-spawn.sh <id> --secondmate` route its dead and missing endpoints use, whose host-local `launch` verb routes the drift into `relaunch` with the profile resolved once.
