...
The tracked `.no-mistakes.yaml` sets `test.evidence.store_in_repo: true`, sets `test.instructions`, and pins `commands.lint` to `bin/fm-lint.sh`, the same owner CI invokes.
...
`test.instructions` steers the Test step's own judgment toward running only the tests that exercise the branch's changed behavior, leaving full deterministic regression coverage to CI, without pinning a command for it to run.
It also directs the Test step to never set an `FM_*_E2E` variable, `FM_LIVE=1`, or otherwise activate a test that self-gates as opt-in live (`fm_live_gate` in `tests/lib.sh`, the `live-harness-optin` family in `bin/fm-test-run.sh`); those stay available only for a deliberate maintainer run outside routine validation.
...
`test.instructions` steers an agent that executes code, so it carries the same trust property and is likewise honored only from the default-branch copy.
