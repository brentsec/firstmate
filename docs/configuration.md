...
It also directs the Test step never to activate a test whose own `fm_live_gate` (`tests/lib.sh`) classifies it as `opt-in`, leaving `default-on` guards eligible like any other test; those opt-in checks stay available only for a deliberate maintainer run outside routine validation.
