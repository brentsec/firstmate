...
It also directs the Test step never to activate a test that self-gates as opt-in live (`fm_live_gate` in `tests/lib.sh`, the `live-harness-optin` family in `bin/fm-test-run.sh`); those stay available only for a deliberate maintainer run outside routine validation.
