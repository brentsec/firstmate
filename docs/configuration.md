...
That refusal covers launching only: a file the home cannot parse is not evidence about an already-running process, so existing workers keep their ordinary liveness state and the captain can still stop, interrupt, or relaunch the fleet while the value is repaired.
...
On Herdr, the policy-aware recovery state therefore checks the running Claude process's exact argv against the recorded mode, reports `permission-drift` when that flag is absent, reports `ambiguous` for more than one foreground Claude process or one process carrying both permission flags, and routes one attributed drifted process through the ordinary transactional relaunch in its same endpoint and worktree.
A record with no recorded mode, a failed or malformed process read, or a build that reports no argv is not evidence of drift: the endpoint keeps its proven live state and ordinary lifecycle control stays available.
